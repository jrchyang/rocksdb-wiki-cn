...
如果可以在插入前离线预处理数据。还有一种更快的方法：可以对数据进行排序，并行生成范围不重叠的 SST 文件，然后批量加载 SST 文件。参见 [[Creating-and-Ingesting-SST-files]]
...
A：更好的方法是使用 SstFileWriter，它允许你直接创建 RocksDB SST 文件并将其添加到 RocksDB 数据库中。不过，如果你要把 SST 文件添加到现有的 RocksDB 数据库中，那么它的键范围就不能与数据库重叠。[[Creating-and-Ingesting-SST-files]]
...
A：不会。有关快照的工作原理，请参阅 [[Overview#Gets, Iterators and Snapshots]]

Q：使用 DBWithTTL 时，删除过期密钥是否有时间限制？
A：`DBwithTTL` 本身不提供时间上限。过期的 keys 将在任何压实过程中被删除。但是，并不能保证何时开始这种压实。例如，如果某个 key 范围从不更新，那么压实就不太可能应用于该 key 范围。对于分级压实，可以使用定期压实功能来执行某些限制。该功能目前有一个限制：如果写入速度太慢，以至于从未触发过内存表刷新，那么也不会触发定期压实。

Q：如果我删除了一个列族，但我还没有删除列族句柄，我还能用它来访问数据吗？
A：可以。`DropColumnFamily()` 只会将指定的列族标记为已丢弃，在其引用计数归零并标记为已丢弃之前，它不会被丢弃。

Q：为什么我只发出写入请求，RocksDB 却从磁盘读取数据？
A：这种 IO 读取来自压实。RocksDB 压实从一个或多个 SST 文件读取数据，执行类似合并排序的操作，生成新的 SST 文件，并删除输入的旧 SST 文件。

Q：block_size 是在压缩之前还是之后？
A：block_size 表示压缩前的大小。

Q：使用 `options.prefix_extractor` 后，我有时会看到错误的结果。这是为什么？
A：`options.prefix_extractor` 有一些限制。如果使用前缀迭代，则不支持 `Prev()` 或 `SeekToLast()`，许多操作也不支持 `SeekToFirst()`。一个常见的错误是通过调用 `Seek()`，然后再调用 `Prev()`来查找前缀的最后一个键，但这种方法并不支持。目前无法通过前缀迭代找到前缀的最后一个键。此外，在查找完前缀后，也无法继续迭代键。在需要进行这些操作的地方，可以尝试设置 `ReadOptions.total_order_seek = true` 来禁用前缀迭代。
...
A：主要区别在于备份、原子写入和写入性能。
	...
	2. 使用多列族的优势：
		- 在一个数据库的多个列族中，写入批次是原子的，而使用多个 RocksDB 数据库无法实现这一点
...
A：使用 `MultiGet()` 有潜在的性能优势。请参见 [[MultiGet-Performance]]
...
Q：RocksDB 支持副本吗？
A：不，RocksDB并不直接支持副本。不过，它提供了一些应用程序接口（API），可用作支持副本的构件。例如，`GetUpdatesSince()` 可以让开发人员遍历特定时间点以来的所有更新。请参见 [[Replication-Helpers]]
...
A：不，RocksDB 没有列。关于什么是列族，请参见 [[Column-Families]]
...
Q：我应该使用哪种压缩类型？
A：为了获得良好的性能，所有级别都应从 LZ4（或 Snappy，如果 LZ4 不可用）开始。如果想进一步缩小数据大小，可尝试在最底层使用 ZStandard（如果没有 ZStandard，则使用 Zlib）。参见 [[Setup-Options-and-Basic-Tuning#压缩（Compression）]]

Q：如果没有键被删除或覆盖，是否需要压缩？
A：即使不需要清除过期数据，也需要进行压缩以确保读取性能。
...
Q：RocksJava 支持所有功能吗？
A：我们正在努力使 RocksJava 功能兼容。不过，如果您发现缺少某些功能，欢迎您提交拉取请求。

## 备份（Backup）

Q：我可以保存 RocksDB 的 "快照"，然后将数据库状态回滚到它吗？
A：是的，通过 [[How-to-Backup-RocksDB|备份引擎]] 或 [[Checkpoints|检查点]]。
...
A：迭代器在内存中持有数据块和内存表。每个迭代器持有的资源有：
	1. 迭代器当前指向的数据块。参见 [[Memory-usage-in-RocksDB#迭代器固定的块（Blocks pinned by iterators）|迭代器固定的块]]
...
Q：我该如何实现多个数据分片/分区。
A：每个分片/分区可以使用一个 RocksDB 数据库。多个 RocksDB 实例可以作为独立进程或在单个进程中运行。在单个进程中使用多个 RocksDB 实例时，这些 RocksDB 实例之间可以共享某些资源（如线程池、块缓存、速率限制器等）（参见 [[Overview#在同一进程中支持多个嵌入式数据库]]）。

Q：数据库操作因超出空间而失败。如何才能解除阻塞？
A：首先清理一些可用空间。一旦有足够的可用空间，数据库就会自动开始接受操作。唯一的例外情况是启用了 2PC 且 WAL 同步失败（在这种情况下，数据库需要重新打开）。有关详细信息，请参阅 [[Background-Error-Handling]]
